...
const QuickStats: React.FC<QuickStatsProps> = ({ stats, mousePosition, colorMap }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    {stats.map((stat) => (
      <div
        key={stat.label}
        className="group relative bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 transform md:hover:scale-105 md:hover:-translate-y-1 border border-white/50 overflow-hidden"
        style={{
